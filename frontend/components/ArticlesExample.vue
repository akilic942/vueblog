    ...
    <div class="uk-child-width-1-2" uk-grid>
      <div>
        <router-link v-for="article in leftArticles"
                     ...
                     :to="{ name: 'articles-id', params: { id: article.attributes.id } }"
                     class="uk-link-reset">
          <div class="uk-card uk-card-muted">
            <div v-if="article.attributes.image" class="uk-card-media-top">
              <img :src="api_url + article.attributes.image.data[0].attributes.url"
                   alt=""
                   height="100"/>
            </div>
            <div class="uk-card-body">
              <span v-for="category in article.attributes.categories.data"
                    id="category2"
                    :key="category.id"
                    class="uk-text-uppercase"
              >{{ category.attributes.name }}
              </span>
              <p id="title2" class="uk-text-large">
...
export default {
  ...
  computed: {
    ...
    rightArticles() {
      console.log(
        typeof this.articles[1].attributes.image.data[0] !== "undefined"
      );
